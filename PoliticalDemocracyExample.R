...
start <- rep(1.5,31)
start[26:31] <- 0 #If covariances are equal to variances, matrix will not be invertible

semCustom <- function(start, dat, wish = FALSE, estimator = "ML", parameterNumb = c(11, 1, 2, 11, 6)){
...
#  
  ...
  setFunction <- function(start, dat, wishart = wish, fit = estimator){
    ...
    # The theta matrix represents our (estimated) error variances and covariances. This is iteratively changed to minimize our fit function
    ...
    # The Phi matrix is a covariance/correlation matrix between exogenous latent variables
    # The Psi matrix is a covariance/correlation matrix between endogenous latent variables
    psi <- diag(1,2)
    phi <- diag(1)
  ...
  printResult <- function(output.estimate, output.df, output.chi, output.indices){
    ...
    parameterName = c("loading", "Beta", "Gamma", "Variance", "Covariance")
...
# 
  ...
  output.estimate$par[5:8] <- output.estimate$par[5:8]/output.estimate$par[5]
  output.estimate$par[9:11] <- output.estimate$par[9:11]/output.estimate$par[9]
...
res <- semCustom(start, PoliticalDemocracy, wish = FALSE, estimator = "ML", parameterNumb = c(11, 1, 2, 11, 6))
